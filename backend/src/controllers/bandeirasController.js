    ...
    try {
        ...
        if (bandeiraExistente) {
            ...
            return res.status(200).json({
                mensagem: "Imagem da bandeira atualizada com sucesso",
                bandeira: bandeiraExistente,
                ok: true,
            })
        }
        ...
        res.status(201).json({
            mensagem: "Bandeira criada com sucesso",
...
export const editarBandeira = async (req, res) => {
    try {
        const { id } = req.params
        const { nome, logo_bandeira } = req.body

        console.log("Dados recebidos para edição da bandeira:", req.body)
        ...
        // Se há um arquivo no upload (via multer)
        if (req.file) {
        ...
        // Se há logo_bandeira no body (via JSON do modal)
        else if (logo_bandeira && logo_bandeira !== bandeira.logo_bandeira) {
            console.log("Logo_bandeira recebida via JSON:", logo_bandeira)
            bandeira.logo_bandeira = logo_bandeira
        }
        ...
        res.json({
            mensagem: "Bandeira atualizada com sucesso",
            bandeira,
            ok: true,
        })
    } catch (error) {
